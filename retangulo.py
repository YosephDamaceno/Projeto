...
def iniciar_figura_nova(event): 
    ...
    if tipo_figura_var.get() == 'Linha':
        ...
        figura_nova = ("retangulo", ((event.x, event.y, event.x, event.y))) ###mexi aqui###
    else:
...
def incompleta(figura):
    ...
    if fig == "linha":
        ...
        return (values[0], values[1]) == (values[0], values[3])
    else : # fig == "rabisco"
        return len(values) <= 1
...
# label
label = ttk.Label(frame,  text='Escolha se vai desenhar linha, Rabisco ou retangulo:') 
